...
module.exports = {
  async search(ctx) {
    ...
    const page = parseInt(ctx.request.query.page) || 1;
    const pageSize = parseInt(ctx.request.query.pageSize) || 10;
    const isNewsSearch = ctx.request.query.newsOnly === 'true';

    let results = [];

    if (isNewsSearch) {
      if (!query) {
        results = await strapi.entityService.findMany('api::news-collection.news-collection', {
        ...
        results = await strapi.service("api::search.search").search(query, ["api::news-collection.news-collection"]);
      }
    } else {
      const searchTypes = types
        ? types.split(",")
      ...
      results = await strapi.service("api::search.search").search(query, searchTypes);
    }

    const publishedResults = results.filter((item) => item.publishedAt);
    const paginatedResults = publishedResults.slice((page - 1) * pageSize, page * pageSize);
